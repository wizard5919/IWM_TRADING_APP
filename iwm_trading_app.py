...
class IWMTradingPlan:
    ...
    def get_premarket_data(self) -> Dict[str, float]:
        """Get premarket high and low data."""
        try:
            ticker = self._get_ticker_data()
            premarket = ticker.history(period="1d", interval="5m")
            if not premarket.empty:
                return {
                    'premarket_high': premarket['High'].max(),
                    'premarket_low': premarket['Low'].min()
                }
            logger.warning("No premarket data available")
            return {}
        except Exception as e:
            logger.error(f"Premarket data error: {str(e)}")
            return {}
